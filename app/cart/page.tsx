      ...
      <div className='overflow-x-auto'>
        <div className='max-w-4xl mx-auto min-w-fit'>
          ...
          {items.length === 0 ? (
            ...
            <ul className="w-full">
              {items.map(item => (
                <li key={item.id + item.selectedColor + item.selectedSize} className='grid place-items-center grid-cols-[20rem_1fr_1fr_1fr_1fr_1fr] border-b border-gray-400'>
                  <div className='p-2 my-4 flex gap-2 self-start justify-self-start'>
                    <Image className='h-fit rounded-xl' src={item.images[0]} width={100} height={100} alt={item.name} />
